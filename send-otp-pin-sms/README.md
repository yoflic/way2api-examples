...
| | |
|---|---|
| **Endpoint** | `POST https://app.way2api.com/api/v1/aws/send_otp-pin` |
| **Category** | Telecom |
| **Coverage** | Global |
| **Authentication** | `Authorization: Bearer YOUR_API_KEY` |
| **API code** | `send_otppin_sms` |
| **Full documentation** | [https://app.way2api.com/documentation/send-otp-pin-sms](https://app.way2api.com/documentation/send-otp-pin-sms) |
...
|---|---|---|---|
| `number` | string | Yes | Country code + mobile number. Currently supported: +91 (India). Example: 919876543210. |
| `content` | string | Yes | Content type: "OTP" or "PIN". |
| `value` | string | Yes | The OTP/PIN code to send (4 to 8 digits only). |
| `purpose` | string | Yes | Purpose of the SMS. Allowed: login, registration, password_reset, transaction, account_verification, two_factor_auth, payment_confirmation, order_confirmation, profile_update, withdrawal. |
| `share_with` | string | Yes | Sharing instruction. Allowed: no_one, executive, manager, support_team, admin. |
...
ENDPOINT = "https://app.way2api.com/api/v1/aws/send_otp-pin"
...
const ENDPOINT = "https://app.way2api.com/api/v1/aws/send_otp-pin";
...
class Way2ApiExample {
    ...
    static final String ENDPOINT = "https://app.way2api.com/api/v1/aws/send_otp-pin";
    ...
    public static void main(String[] args) throws Exception {
        ...
        String payload = "{\"value\":\"5289\",\"number\":\"919876543210\",\"content\":\"OTP\",\"purpose\":\"login\",\"share_with\":\"no_one\"}";
...
class Program
{
    const string Endpoint = "https://app.way2api.com/api/v1/aws/send_otp-pin";

    static async Task<int> Main()
    {
        var apiKey = Environment.GetEnvironmentVariable("WAY2API_KEY") ?? "YOUR_API_KEY";
        var payload = "{\"value\":\"5289\",\"number\":\"919876543210\",\"content\":\"OTP\",\"purpose\":\"login\",\"share_with\":\"no_one\"}";
...
ENDPOINT = 'https://app.way2api.com/api/v1/aws/send_otp-pin'.freeze
...
const val ENDPOINT = "https://app.way2api.com/api/v1/aws/send_otp-pin"

fun main() {
    val apiKey = System.getenv("WAY2API_KEY") ?: "YOUR_API_KEY"
    val payload = "{\"value\":\"5289\",\"number\":\"919876543210\",\"content\":\"OTP\",\"purpose\":\"login\",\"share_with\":\"no_one\"}"
...
let endpoint = URL(string: "https://app.way2api.com/api/v1/aws/send_otp-pin")!
...
The full status and error-code reference for this API is at [https://app.way2api.com/documentation/send-otp-pin-sms/errors](https://app.way2api.com/documentation/send-otp-pin-sms/errors).

## Common use cases

- **Login and transaction OTP** — Send a one-time code to confirm a login, a payment or a profile change.
- **Account recovery** — Deliver a reset PIN to a verified mobile number during password recovery.
- **Phone number ownership** — Prove a user controls the number they entered before you attach it to an account.
- **International delivery** — Send transactional codes to mobile numbers worldwide from one endpoint.

## Rate limits

Limits apply per API key, per service, on a one-minute sliding window. Exceeding them returns HTTP `429` with a `Retry-After` header. Current limits for this API are published at [https://app.way2api.com/documentation/send-otp-pin-sms/rate-limits](https://app.way2api.com/documentation/send-otp-pin-sms/rate-limits).

## More

- [Full documentation](https://app.way2api.com/documentation/send-otp-pin-sms)
- [Request reference](https://app.way2api.com/documentation/send-otp-pin-sms/request)
- [Response reference](https://app.way2api.com/documentation/send-otp-pin-sms/response)
- [All Way2API documentation](https://app.way2api.com/documentation)

---

Part of [way2api-examples](../README.md). Slug: `send-otp-pin-sms`
